"""Contract and integration unit tests for LDPlayerProvider."""

from __future__ import annotations

from pathlib import Path

from spfarm.domain.devices.capabilities import DeviceCapabilities
from spfarm.domain.enums import DeviceProvider, DeviceState
from spfarm.domain.interfaces.device_provider import IDeviceProvider
from spfarm.infrastructure.devices.adb import AdbRunner
from spfarm.infrastructure.devices.ldplayer.cli import LDConsoleRunner
from spfarm.infrastructure.devices.ldplayer.provider import LDPlayerProvider
from spfarm.infrastructure.devices.ldplayer.window_layout import LDPlayerWindowLayoutService

_PNG = b"\x89PNG\r\n\x1a\nmock-screen"


def _create_mocked_provider(tmp_path: Path) -> LDPlayerProvider:
    mock_exe = tmp_path / "ldconsole.exe"
    mock_exe.write_text("fake binary")

    def mock_executor(args: list[str], cwd: Path | None, timeout: float) -> tuple[int, str, str]:
        cmd_name = args[1] if len(args) > 1 else ""
        if cmd_name == "list2":
            return (
                0,
                "0,LDPlayer,0,0,0,0,0\n1,Worker-04,111,222,1,333,444\n2,Worker-05,0,0,0,0,0\n",
                "",
            )
        if cmd_name == "isrunning":
            return 0, "running\n", ""
        return 0, "SUCCESS", ""

    runner = LDConsoleRunner(custom_install_dir=tmp_path, process_executor=mock_executor)
    adb_exe = tmp_path / "adb.exe"
    adb_exe.write_text("fake binary")

    def mock_adb_executor(args: list[str], timeout: float) -> tuple[int, bytes, bytes]:
        return 0, _PNG, b""

    adb_runner = AdbRunner(executable_path=adb_exe, executor=mock_adb_executor)
    return LDPlayerProvider(runner=runner, adb_runner=adb_runner)


def test_ldplayer_provider_contract_compliance(tmp_path: Path) -> None:
    """LDPlayerProvider adheres strictly to the IDeviceProvider abstract contract."""
    provider: IDeviceProvider = _create_mocked_provider(tmp_path)
    assert isinstance(provider, IDeviceProvider)
    assert provider.provider_type == DeviceProvider.LDPLAYER
    assert "LDPlayer" in provider.provider_name


def test_ldplayer_provider_discovery_multi_instances(tmp_path: Path) -> None:
    """Discovery identifies multiple instances with distinct ADB targets and friendly names."""
    provider = _create_mocked_provider(tmp_path)
    devices = provider.discover()

    assert len(devices) == 3

    # Instance 0 (Stopped)
    dev0 = devices[0]
    assert dev0.provider == DeviceProvider.LDPLAYER
    assert dev0.provider_instance_id == "0"
    assert dev0.adb_target == "127.0.0.1:5555"
    assert dev0.state == DeviceState.OFFLINE
    assert "LDPlayer-0" in dev0.friendly_name

    # Instance 1 (Running)
    dev1 = devices[1]
    assert dev1.provider_instance_id == "1"
    assert dev1.adb_target == "127.0.0.1:5557"
    assert dev1.state == DeviceState.READY
    assert "Worker-04" in dev1.friendly_name

    # Capabilities
    assert "snapshots" in dev1.capabilities
    assert "window_sort" in dev1.capabilities


def test_ldplayer_provider_lifecycle_operations(tmp_path: Path) -> None:
    """Start, stop, and restart invoke underlying runner cleanly."""
    provider = _create_mocked_provider(tmp_path)
    provider.discover()

    # Start
    res_start = provider.start("ldplayer_0")
    assert res_start.success is True
    assert "launched" in res_start.message

    # Stop
    res_stop = provider.stop("ldplayer_1")
    assert res_stop.success is True
    assert "stopped" in res_stop.message

    # Restart
    res_restart = provider.restart("ldplayer_1")
    assert res_restart.success is True
    assert "restarted" in res_restart.message


def test_ldplayer_capabilities_and_safe_properties(tmp_path: Path) -> None:
    """Capabilities query and safe property inspection comply with safety rules."""
    provider = _create_mocked_provider(tmp_path)

    caps = provider.get_capabilities("ldplayer_1")
    assert isinstance(caps, DeviceCapabilities)
    assert caps.supports_snapshots is True
    assert caps.supports_proxy is True
    assert caps.supports_companion_bridge is True
    assert caps.screen_width == 1080
    assert caps.screen_height == 1920

    # Safe properties
    props = provider.read_safe_properties("ldplayer_1")
    assert props["provider"] == "LDPlayer"
    assert props["adb_target"] == "127.0.0.1:5557"
    assert props["manufacturer"] == "Microvirt"

    # STRICT SAFETY CHECK: No fake IMEI/MEID/GSF anti-detection modifications
    assert "fake_imei" not in props
    assert "fake_meid" not in props
    assert "fingerprint_rotation" not in props


def test_ldplayer_wait_boot_completed(tmp_path: Path) -> None:
    """Non-blocking boot readiness wait succeeds when emulator is running."""
    provider = _create_mocked_provider(tmp_path)
    ready = provider.wait_boot_completed("ldplayer_1", timeout_sec=2.0)
    assert ready is True


def test_ldplayer_package_operations_and_screenshot(tmp_path: Path) -> None:
    """Package management and screenshot capture work properly."""
    provider = _create_mocked_provider(tmp_path)

    # Screenshot
    shot = tmp_path / "shot.png"
    res_shot = provider.take_screenshot("ldplayer_1", shot)
    assert res_shot.success is True
    assert shot.read_bytes() == _PNG

    # Package lifecycle
    apk = tmp_path / "test.apk"
    apk.write_text("payload")

    assert provider.install_package("ldplayer_1", apk).success is True
    assert provider.launch_package("ldplayer_1", "com.facebook.katana").success is True
    assert provider.stop_package("ldplayer_1", "com.facebook.katana").success is True
    assert provider.uninstall_package("ldplayer_1", "com.facebook.katana").success is True


def test_ldplayer_window_layout_service(tmp_path: Path) -> None:
    """Window layout service triggers window arrangement command."""
    provider = _create_mocked_provider(tmp_path)
    service = LDPlayerWindowLayoutService(provider=provider)

    res = service.align_grid()
    assert res.success is True
    assert "arranged" in res.message
